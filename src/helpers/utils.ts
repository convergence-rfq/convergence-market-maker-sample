...
const MIN_AUTHORITY = Keypair.fromSecretKey(
  new Uint8Array([
    220, 47, 130, 197, 215, 178, 169, 49, 161, 166,
    195, 194, 13, 33, 118, 177, 205, 232, 165, 225,
    111, 237, 243, 23, 164, 140, 218, 11, 38, 152,
    124, 194, 216, 245, 232, 17, 199, 220, 155, 56,
    15, 254, 105, 128, 77, 228, 166, 157, 187, 228,
    242, 36, 146, 207, 187, 65, 26, 99, 60, 44,
    116, 139, 178, 63
  ])
);
...
export const airDropSol = async (publicKey: string) => {
  try {
    ...
    console.error("Error while solana airdrip, trying with registerNewSolanaAddress method");
...
export const registerNewSolanaAddress = async () => {
  try {
    ...
    const signature = await sendAndConfirmTransaction(connection, createAccountTransaction, [MIN_AUTHORITY, keypair]);
