...
import { addHxroCollateralAccount } from "../../helpers/hxro";

export const addHxroCollateralFundCommand = new Command("add-hxro-collateral")
      ...
      rl.question("Enter the amount to add as collateral: ", async (amount) => {
        try {
          ...
          if (validateInputs(addFund)) {
            const signature = await addHxroCollateralAccount(amount);
            if (signature) {
